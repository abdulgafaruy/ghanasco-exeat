...
export default function App() {
  ...
  const [houseStudentCount, setHouseStudentCount] = useState(0);
  ...
  // Fetch student count for housemaster's house
  const fetchHouseStudentCount = async (token, houseId) => {
    try {
      if (!houseId) return;
      const response = await fetch(`${API_URL}/houses/${houseId}/students`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success) {
        setHouseStudentCount(data.count);
      }
    } catch (error) {
      console.error('Error fetching house student count:', error);
    }
  };
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      console.log('📤 Uploading CSV with', fileContent.split('\n').length - 1, 'rows');
      ...
      if (data.success) {
        ...
        let message = `✅ Successfully uploaded ${data.successCount} students!`;
        if (data.failureCount > 0) {
          message += `\n\n❌ Failed: ${data.failureCount} rows\n\nFirst errors:\n`;
  ...
  // Fetch student count for housemaster
  useEffect(() => {
    if (user && (user.role === 'housemaster' || user.role === 'senior_housemaster')) {
      fetchHouseStudentCount(localStorage.getItem('token'), user.house_id);
    }
  }, [user]);
        ...
        {activeTab === 'dashboard' && (
          <div>
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '15px', marginBottom: '30px'}}>
              ...
              <div style={{background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', color: 'white', padding: '20px', borderRadius: '8px', textAlign: 'center', boxShadow: '0 4px 12px rgba(245,158,11,0.3)'}}>
              ...
              {(user.role === 'housemaster' || user.role === 'senior_housemaster') && (
                <div style={{background: 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)', color: 'white', padding: '20px', borderRadius: '8px', textAlign: 'center', boxShadow: '0 4px 12px rgba(139,92,246,0.3)'}}>
                  <p style={{margin: '0 0 8px 0', fontSize: '24px', fontWeight: '700'}}>👥 {houseStudentCount}</p>
                  <p style={{margin: 0, fontSize: '12px'}}>Students in House</p>
                </div>
              )}
            </div>
