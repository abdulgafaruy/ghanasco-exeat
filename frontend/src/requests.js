...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    let query = `
      SELECT r.*, 
             u.first_name, u.last_name, u.email, u.phone, u.house_id as user_house_id,
...
// PUT approve request
router.put('/:id/approve', authenticate, authorize(['housemaster', 'senior_housemaster', 'admin']), async (req, res) => {
  try {
    // Get request details
    ...
    await pool.query(
      'INSERT INTO audit_logs (user_id, action, details) VALUES ($1, $2, $3)',
      [req.user.id, 'APPROVE_REQUEST', `Approved exeat for ${request.first_name} ${request.last_name} to ${request.destination}`]
    );
...
// PUT reject request
router.put('/:id/reject', authenticate, authorize(['housemaster', 'senior_housemaster', 'admin']), async (req, res) => {
...
// PUT cancel request (student only)
router.put('/:id/cancel', authenticate, authorize(['student']), async (req, res) => {
  try {
    const result = await pool.query(
